function [MRb, CRb, KRb, sym_var] = lag_eq(mode,blades)
% lag_eq Symbolic development of the blade GR equations of motion
%
% Input:
%       * mode : lead-lag damping mechanism
%       * blades : number of blades
% Output:
%       * MRb : Rotating reference frame blade inertia matrix
%       * CRb : Rotating reference frame blade damping matrix
%       * KRb : Rotating reference frame blade stiffness matrix
%       * sym_var : Symbolic variables to be substituted for the real
...
if mode == 0
    ...
    CRb = subs(CRb,c_xic,2*c_xic);
    CRb = subs(CRb,c_ed,c_xic);
end
...
sym_var = [S_b n_b m_b omega I_b c_xi k_xi e];
end